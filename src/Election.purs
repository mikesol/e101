...
import FRP.Behavior.Audio (AV(..), AudioParameter, AudioUnit, CanvasInfo(..), evalPiecewise, gainT_, gainT_', gain_', panner_, playBufWithOffset_, runInBrowser, speaker)
...
rm :: Int -> Int -> Number -> Number -> Drawing
rm c r w h =
  let
    rw = w / (toNumber c)
...
fax :: CanvasInfo -> Number -> Array (Number -> List (Drawing))
