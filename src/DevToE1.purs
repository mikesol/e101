...
import Prelude
import Color (rgb)
import Data.Array (catMaybes, head, index, range, take)
...
import FRP.Behavior.Audio (AV(..), AudioParameter, CanvasInfo(..), audioWorkletProcessor_, defaultExporter, dup2, evalPiecewise, g'add_, g'delay_, g'gain_, gain_, gain_', graph, loopBuf, loopBuf_, makePeriodicWave, pannerMono, periodicOsc, playBuf_, runInBrowser_, speaker)
      ...
      \ctx _ res rej -> do
        ...
        pw2 <-
          makePeriodicWave ctx
            (0.5 +> -0.3 +> empty)
            (0.2 +> -0.7 +> empty)
        res $ O.union (O.singleton "smooth" pw) (O.singleton "rich" pw2)
